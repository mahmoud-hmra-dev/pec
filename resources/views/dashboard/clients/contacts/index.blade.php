...
<div class="p-4">
    <div class="pt-2">
        <button
            ...
            href="javascript:" onclick="openAdd({!! $client_id !!})">
...
@component('dashboard.clients.contacts.form')
@endcomponent
...
<script>
    $(function () {
        var table = $('#contacts').DataTable({
            processing: true,
            serverSide: false,
            ajax: '{!! route('clients.contacts.index',$client_id) !!}',
            dom: 'Blfrtip',
            responsive:true,
            buttons: [
                'copyHtml5',
        ...
        table.on('click', '.delete', function () {
            ...
            deleteOpr(data.id, `dashboard/users/clients/` + data.client.id+ `/contacts/`+ data.id, table);
        });
    ...
    openAdd = (clientId) => {
        ...
        $form.attr('action', `dashboard/users/clients/`+ clientId+'/contacts');
    ...
    openEdit = (data) => {
        ...
        $form.attr('action', `dashboard/users/clients/` + data.client.id+ `/contacts/`+ data.id);
